...
class AttributeGeneral:
    ...
    def get_rq(self, best_path, time_col, t1, t2, pcol):
        ...
        sort_func = {
            'classification': lambda x, key: (self.df[self.df[key] == x][self.df[time_col] == t2][pcol].nunique() - self.df[self.df[key] == x][self.df[time_col] == t1][pcol].nunique())/get_sign(self.df[self.df[time_col] == t2][pcol].nunique(), self.df[self.df[time_col] == t1][pcol].nunique()),
            'regression': lambda x, key: (self.df[self.df[key] == x][self.df[time_col] == t2][pcol].sum() - self.df[self.df[key] == x][self.df[time_col] == t1][pcol].sum())/get_sign(self.df[self.df[time_col] == t2][pcol].sum(), self.df[self.df[time_col] == t1][pcol].sum())
        }[self.task_type]
        ...
        for q in query:
            if q.startswith('~'):
                ...
                value = {i: sort_func(i, key = key) for i in value_b}
                rq[key] = value
            elif 'in' in q:
                key = q.split(' in ')[0]
                value = eval(q.split(' in ')[-1].strip(')').strip('('))
                value = {i: sort_func(i, key = key) for i in value}
                rq[key] = value
            elif '<=' in q:
                key = q.split(' <= ')[0]
    ...
    def __call__(self, time_col: str, comp_time: list, pcol: str = None, depth: int=5):
        ...
        if self.task_type == 'classification':
            ...
            paths = self.find_path(self.tree_df, target=time_col, max_depth=depth)
            for path in paths:
